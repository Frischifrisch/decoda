...
LF = "\n"
CRLF = "\r\n"
NATIVE = CRLF if sys.platform == "win32" else LF
...
def CopyWithInsertion(input, commentPrefix, retainDefs, eolType, *lists):
    ...
    for line in input.splitlines(0):
        isStartGenerated = line.startswith(f"{commentPrefix}++Autogenerated")
        ...
        if isStartGenerated:
            ...
            copying = 0
            definition = ""
        elif not copying and line.startswith(f"{commentPrefix}**"):
            if retainDefs:
                output.append(line)
            ...
            output.append(out)
        elif line.startswith(f"{commentPrefix}--Autogenerated"):
            copying = 1
            if retainDefs:
                output.append(line)
